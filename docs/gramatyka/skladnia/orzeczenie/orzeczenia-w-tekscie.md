# Orzeczenia w tekście

<div class="predicate-lesson" markdown>

<div class="predicate-lead" markdown>

W prawdziwym tekście orzeczenia nie występują jako oderwana lista form. Prowadzą wydarzenia, opisują tło, przypisują bohaterom cechy i role, pokazują konieczność lub możliwość, a także budują zdania złożone. Poniższa opowieść pozwala obserwować wszystkie najważniejsze typy orzeczeń w jednym spójnym świecie.

</div>

<div class="predicate-reading">
  <div class="predicate-reading-header">
    <h2>Nocny sygnał z obserwatorium</h2>
    <p>Kolory i kształty pokazują typ centrum orzeczeniowego. Komentarze po scenach wyjaśniają, jak orzeczenia organizują tekst.</p>
    <div class="predicate-reading-legend">
      <span class="predicate-mark predicate-mark--verbal">czasownikowe</span>
      <span class="predicate-mark predicate-mark--nominal">imienne</span>
      <span class="predicate-mark predicate-mark--compound">złożone</span>
      <span class="predicate-mark predicate-mark--impersonal">bezosobowe</span>
      <span class="predicate-mark predicate-mark--negation">przeczone</span>
    </div>
  </div>
  <div class="predicate-reading-text" markdown>

<div class="predicate-scene-break">1. Ostatnia kontrola</div>

Stare obserwatorium <span class="predicate-mark predicate-mark--nominal">było prawie gotowe</span> do ponownego otwarcia. Przez kilka miesięcy technicy <span class="predicate-mark predicate-mark--verbal">naprawiali</span> kopułę, <span class="predicate-mark predicate-mark--verbal">czyścili</span> teleskop i <span class="predicate-mark predicate-mark--verbal">wymieniali</span> przewody w głównej sali.

Wieczorem przed pierwszym pokazem Lena <span class="predicate-mark predicate-mark--compound">miała przeprowadzić</span> ostatnią kontrolę. <span class="predicate-mark predicate-mark--compound">Musiała sprawdzić</span> zasilanie, <span class="predicate-mark predicate-mark--compound">powinna była uruchomić</span> mechanizm kopuły i <span class="predicate-mark predicate-mark--compound">miała zapisać</span> wyniki w dzienniku.

Kiedy <span class="predicate-mark predicate-mark--verbal">weszła</span> do budynku, wszystkie pomieszczenia <span class="predicate-mark predicate-mark--nominal">były ciche</span>. Na korytarzu <span class="predicate-mark predicate-mark--verbal">paliła się</span> tylko jedna lampa, a z sali sterowania <span class="predicate-mark predicate-mark--verbal">dochodził</span> jednostajny szum wentylatora.

— Wszystko <span class="predicate-mark predicate-mark--nominal">wydaje się spokojne</span> — powiedziała Lena, choć sama <span class="predicate-mark predicate-mark--nominal">nie była całkiem pewna</span>.

<div class="predicate-reading__commentary" markdown>
<strong>Jak orzeczenia otwierają opowieść?</strong>

**Było prawie gotowe, były ciche, wydaje się spokojne, nie była pewna** opisują stan miejsca i bohaterki. Szereg **naprawiali, czyścili, wymieniali** przedstawia wcześniejsze prace. Konstrukcje **miała przeprowadzić, musiała sprawdzić, powinna była uruchomić, miała zapisać** budują plan i obowiązki. Zdanie z **kiedy weszła** ma osobne centrum podrzędne, które ustawia czas wydarzenia głównego.
</div>

<div class="predicate-scene-break">2. Światło w zamkniętej kopule</div>

Lena <span class="predicate-mark predicate-mark--verbal">włączyła</span> komputer i <span class="predicate-mark predicate-mark--verbal">otworzyła</span> program sterujący. System <span class="predicate-mark predicate-mark--verbal">działał</span> poprawnie, ale na planie budynku jedna kontrolka <span class="predicate-mark predicate-mark--verbal">migała</span> na czerwono.

Kontrolka <span class="predicate-mark predicate-mark--verbal">wskazywała</span> zamkniętą galerię pod kopułą. Przed wejściem do tej części budynku <span class="predicate-mark predicate-mark--impersonal">należało wyłączyć</span> główny silnik, dlatego Lena <span class="predicate-mark predicate-mark--compound">musiała wrócić</span> do panelu zasilania.

Nagle w głośniku <span class="predicate-mark predicate-mark--impersonal">zatrzeszczało</span>. Potem <span class="predicate-mark predicate-mark--impersonal">usłyszano</span> trzy krótkie sygnały, chociaż system alarmowy oficjalnie <span class="predicate-mark predicate-mark--negation">nie był aktywny</span>.

Lena <span class="predicate-mark predicate-mark--verbal">zadzwoniła</span> do Marka, technika odpowiedzialnego za automatykę.

— Czy alarm <span class="predicate-mark predicate-mark--compound">może uruchomić się</span> sam? — zapytała.

— Nie powinien — odpowiedział Marek. — Ale jeśli kontrolka <span class="predicate-mark predicate-mark--verbal">miga</span>, <span class="predicate-mark predicate-mark--compound">musisz sprawdzić</span> czujnik przy kopule.

<div class="predicate-reading__commentary" markdown>
<strong>Co zmienia się w drugiej scenie?</strong>

Orzeczenia czasownikowe prowadzą serię konkretnych działań: **włączyła, otworzyła, wskazywała, zadzwoniła**. Formy bezosobowe **należało wyłączyć, zatrzeszczało, usłyszano** odsuwają wykonawcę albo opisują zjawisko. **Nie był aktywny** przeczy stanowi systemu. Pytanie **może uruchomić się** i polecenie **musisz sprawdzić** wprowadzają modalność. Zdanie warunkowe z **jeśli kontrolka miga** zawiera własne centrum podrzędne.
</div>

<div class="predicate-scene-break">3. Drzwi, które nie powinny być otwarte</div>

Po wyłączeniu silnika Lena <span class="predicate-mark predicate-mark--verbal">weszła</span> po wąskich schodach na galerię. Drzwi do pomieszczenia technicznego <span class="predicate-mark predicate-mark--nominal">były uchylone</span>, chociaż poprzedniego dnia <span class="predicate-mark predicate-mark--verbal">zostały zamknięte</span> przez konserwatora.

W środku <span class="predicate-mark predicate-mark--impersonal">było zimno</span>. Na podłodze <span class="predicate-mark predicate-mark--verbal">leżał</span> otwarty zeszyt, a obok niego <span class="predicate-mark predicate-mark--verbal">stała</span> mała latarka. Lena <span class="predicate-mark predicate-mark--negation">nie rozpoznawała</span> tych przedmiotów.

Kiedy <span class="predicate-mark predicate-mark--verbal">podniosła</span> zeszyt, <span class="predicate-mark predicate-mark--verbal">zauważyła</span>, że ostatnia strona <span class="predicate-mark predicate-mark--nominal">była zapisana</span> świeżym atramentem. Widniało na niej jedno zdanie: „Kopuła <span class="predicate-mark predicate-mark--compound">ma otworzyć się</span> o północy”.

Lena <span class="predicate-mark predicate-mark--nominal">stała się niespokojna</span>. Mechanizm <span class="predicate-mark predicate-mark--negation">nie miał pracować</span> tej nocy, a w budynku oprócz niej <span class="predicate-mark predicate-mark--negation">nie powinno być</span> nikogo.

<div class="predicate-reading__commentary" markdown>
<strong>Stan, czynność i niepewność</strong>

**Były uchylone** może opisywać stan drzwi, natomiast **zostały zamknięte** wyraźnie przedstawia wcześniejszą czynność bierną. Konstrukcja **było zimno** jest bezosobowa. Orzeczenia **leżał** i **stała** lokalizują przedmioty. Zdanie **kiedy podniosła, zauważyła, że strona była zapisana** ma trzy centra i dwa poziomy zależności. **Ma otworzyć się** oznacza zaplanowane zdarzenie, **stała się niespokojna** — zmianę stanu, a przeczone konstrukcje **nie miał pracować, nie powinno być** odrzucają oczekiwane możliwości.
</div>

<div class="predicate-scene-break">4. Nagranie sprzed wielu lat</div>

Marek <span class="predicate-mark predicate-mark--verbal">przyjechał</span> po kilkunastu minutach. Gdy <span class="predicate-mark predicate-mark--verbal">zobaczył</span> zeszyt, <span class="predicate-mark predicate-mark--verbal">rozpoznał</span> znak dawnego operatora obserwatorium.

— Ten zeszyt <span class="predicate-mark predicate-mark--nominal">jest częścią starego systemu testowego</span> — wyjaśnił. — Przed każdym automatycznym uruchomieniem <span class="predicate-mark predicate-mark--impersonal">zapisywano</span> w nim plan próby.

Marek <span class="predicate-mark predicate-mark--compound">zaczął sprawdzać</span> przewody pod pulpitem. Po chwili <span class="predicate-mark predicate-mark--verbal">odnalazł</span> niewielki rejestrator, który nadal <span class="predicate-mark predicate-mark--verbal">był podłączony</span> do czujnika kopuły.

Urządzenie <span class="predicate-mark predicate-mark--compound">miało odtwarzać</span> dawne komunikaty podczas testów. Ponieważ jego zegar <span class="predicate-mark predicate-mark--verbal">spóźniał się</span> o kilka lat, system właśnie tej nocy <span class="predicate-mark predicate-mark--compound">zamierzał powtórzyć</span> próbę zapisaną w pamięci.

— Czyli nikt <span class="predicate-mark predicate-mark--negation">nie wszedł</span> do budynku? — zapytała Lena.

— Nie. Drzwi prawdopodobnie <span class="predicate-mark predicate-mark--verbal">otworzyły się</span>, gdy stary mechanizm <span class="predicate-mark predicate-mark--verbal">wykonał</span> pierwszy etap testu.

<div class="predicate-reading__commentary" markdown>
<strong>Jak wyjaśnienie zmienia interpretację wcześniejszych zdań?</strong>

Orzeczenie imienne **jest częścią systemu** identyfikuje zeszyt. Bezosobowe **zapisywano** opisuje dawną procedurę. **Zaczął sprawdzać** wprowadza początek czynności, a zdanie względne **który był podłączony** opisuje rejestrator. **Miało odtwarzać** określa przeznaczenie urządzenia, **zamierzał powtórzyć** personifikuje system i przedstawia zaprogramowany zamiar. W ostatnim zdaniu dwa centra **otworzyły się** oraz **wykonał** tworzą relację czasową.
</div>

<div class="predicate-scene-break">5. Próba przed północą</div>

Do północy <span class="predicate-mark predicate-mark--impersonal">pozostało</span> piętnaście minut. Lena i Marek <span class="predicate-mark predicate-mark--compound">musieli zdecydować</span>, czy <span class="predicate-mark predicate-mark--compound">powinni zatrzymać</span> próbę, czy <span class="predicate-mark predicate-mark--compound">mogą bezpiecznie obserwować</span> działanie systemu.

Po krótkiej naradzie <span class="predicate-mark predicate-mark--verbal">wyłączyli</span> napęd kopuły, ale <span class="predicate-mark predicate-mark--verbal">pozostawili</span> rejestrator podłączony. Chcieli sprawdzić, jakie komunikaty <span class="predicate-mark predicate-mark--verbal">zostały zapisane</span> w jego pamięci.

O północy w głośniku ponownie <span class="predicate-mark predicate-mark--impersonal">zatrzeszczało</span>. Głos dawnego operatora <span class="predicate-mark predicate-mark--verbal">poinformował</span>, że próba <span class="predicate-mark predicate-mark--compound">ma się rozpocząć</span>, a następnie <span class="predicate-mark predicate-mark--impersonal">nakazano sprawdzić</span> blokady bezpieczeństwa.

Ponieważ napęd <span class="predicate-mark predicate-mark--nominal">był odłączony</span>, kopuła <span class="predicate-mark predicate-mark--negation">nie poruszyła się</span>. Rejestrator <span class="predicate-mark predicate-mark--verbal">odtworzył</span> jeszcze dwa komunikaty i <span class="predicate-mark predicate-mark--verbal">wyłączył się</span> dokładnie minutę później.

— Zagadka <span class="predicate-mark predicate-mark--nominal">jest rozwiązana</span> — powiedziała Lena.

— A stary system <span class="predicate-mark predicate-mark--compound">powinien trafić</span> do muzeum techniki — dodał Marek. — Nadal <span class="predicate-mark predicate-mark--compound">potrafi zaskoczyć</span> człowieka.

<div class="predicate-reading__commentary" markdown>
<strong>Jak orzeczenia zamykają opowieść?</strong>

Bezosobowe **pozostało piętnaście minut** ustawia czas. Konstrukcje **musieli zdecydować, powinni zatrzymać, mogą obserwować** pokazują wybór i ocenę możliwości. **Zostały zapisane** jest stroną bierną. **Ma się rozpocząć** odtwarza zaplanowany przebieg próby, a **nakazano sprawdzić** łączy bezosobowy nakaz z bezokolicznikiem. Przeczenie **nie poruszyła się** informuje, że oczekiwane zdarzenie nie zaszło. Finałowe **jest rozwiązana** opisuje rezultat, a **powinien trafić, potrafi zaskoczyć** oceniają dalszy los urządzenia.
</div>

  </div>
</div>

## Jak typ orzeczenia wpływa na tekst?

### Orzeczenia czasownikowe prowadzą wydarzenia

> Lena <span class="predicate-mark predicate-mark--verbal">weszła</span>, <span class="predicate-mark predicate-mark--verbal">włączyła</span> komputer i <span class="predicate-mark predicate-mark--verbal">zadzwoniła</span> do Marka.

Kolejne formy dokonane tworzą ciąg następujących po sobie czynności.

### Orzeczenia imienne zatrzymują akcję i opisują stan

> Obserwatorium <span class="predicate-mark predicate-mark--nominal">było ciche</span>.  
> Drzwi <span class="predicate-mark predicate-mark--nominal">były uchylone</span>.  
> Lena <span class="predicate-mark predicate-mark--nominal">stała się niespokojna</span>.

Takie centra opisują tło, wynik lub zmianę cechy.

### Orzeczenia złożone pokazują plan, obowiązek i możliwość

> Lena <span class="predicate-mark predicate-mark--compound">musiała sprawdzić</span> czujnik.  
> System <span class="predicate-mark predicate-mark--compound">miał odtwarzać</span> komunikaty.  
> Technik <span class="predicate-mark predicate-mark--compound">zaczął sprawdzać</span> przewody.

Bezokolicznik nazywa czynność, a pierwszy składnik ją ocenia albo ustawia jej fazę.

### Konstrukcje bezosobowe odsuwają wykonawcę

> W zeszycie <span class="predicate-mark predicate-mark--impersonal">zapisywano</span> plan prób.  
> Przed wejściem <span class="predicate-mark predicate-mark--impersonal">należało wyłączyć</span> silnik.  
> W głośniku <span class="predicate-mark predicate-mark--impersonal">zatrzeszczało</span>.

Tekst skupia się na procedurze albo zjawisku, nie na osobie.

## Jedno zdanie oglądane krok po kroku

> Kiedy Lena weszła na galerię, zauważyła, że drzwi były uchylone, chociaż poprzedniego dnia zostały zamknięte przez konserwatora.

### Centra

- <span class="predicate-mark predicate-mark--verbal">weszła</span>;
- <span class="predicate-mark predicate-mark--verbal">zauważyła</span>;
- <span class="predicate-mark predicate-mark--nominal">były uchylone</span>;
- <span class="predicate-mark predicate-mark--verbal">zostały zamknięte</span>.

### Podział

<div class="predicate-tree">1. Kiedy Lena weszła na galerię,
2. zauważyła,
3. że drzwi były uchylone,
4. chociaż poprzedniego dnia zostały zamknięte przez konserwatora.</div>

### Relacje

<div class="predicate-tree">2. zauważyła
├── kiedy? → 1. kiedy Lena weszła na galerię
└── co? → 3. że drzwi były uchylone
             └── mimo czego? → 4. chociaż zostały zamknięte</div>

Orzeczenia pozwalają zobaczyć cztery warstwy informacji w jednym zdaniu.

## Ta sama sytuacja zapisana różnymi orzeczeniami

<div class="predicate-comparison">
  <div class="predicate-comparison-item"><span class="predicate-comparison-label">czynność</span><p>Technik <span class="predicate-mark predicate-mark--verbal">zamknął</span> drzwi.</p></div>
  <div class="predicate-comparison-item"><span class="predicate-comparison-label">strona bierna</span><p>Drzwi <span class="predicate-mark predicate-mark--verbal">zostały zamknięte</span> przez technika.</p></div>
  <div class="predicate-comparison-item"><span class="predicate-comparison-label">forma bezosobowa</span><p>Drzwi <span class="predicate-mark predicate-mark--impersonal">zamknięto</span>.</p></div>
  <div class="predicate-comparison-item"><span class="predicate-comparison-label">stan</span><p>Drzwi <span class="predicate-mark predicate-mark--nominal">były zamknięte</span>.</p></div>
</div>

Każde zdanie dotyczy zamknięcia drzwi, ale inaczej rozkłada uwagę między wykonawcą, czynnością, wynikiem i stanem.

## Co tracimy, gdy usuniemy orzeczenia?

Fragment:

> Lena, komputer, kontrolka, galeria, zeszyt, rejestrator.

nazywa elementy świata, ale nie tworzy historii. Dopiero orzeczenia pokazują:

- co pojawiło się wcześniej, a co później;
- które elementy działały;
- jaki był stan pomieszczeń i urządzeń;
- co bohaterowie musieli lub mogli zrobić;
- czego nie zrobiono;
- dlaczego kolejne zdania pozostają ze sobą w związku.

## Procedura pracy z dłuższym tekstem

<div class="predicate-analysis">
  <div class="predicate-analysis__title">Czytanie analityczne</div>
  <ol>
    <li>Najpierw przeczytaj fragment bez zatrzymywania się na każdej formie.</li>
    <li>Ustal główne wydarzenia i stany.</li>
    <li>Zaznacz pełne orzeczenia, nie tylko pojedyncze czasowniki.</li>
    <li>Rozróżnij centra czasownikowe, imienne, złożone i bezosobowe.</li>
    <li>W zdaniach złożonych policz centra i wyznacz granice członów.</li>
    <li>Sprawdź, jak czas, aspekt i tryb wpływają na przebieg opowieści.</li>
    <li>Zwróć uwagę, kiedy autor ujawnia wykonawcę, a kiedy go usuwa.</li>
    <li>Na końcu wyjaśnij, po co w danym miejscu wybrano właśnie taki typ orzeczenia.</li>
  </ol>
</div>

<div class="predicate-summary">
<h3>Orzeczenia są silnikiem tekstu</h3>
<ul>
  <li>Orzeczenia czasownikowe budują ciąg wydarzeń i procesów.</li>
  <li>Orzeczenia imienne opisują cechy, stany, role i rezultaty.</li>
  <li>Orzeczenia złożone wyrażają konieczność, możliwość, plan, zamiar i fazę czynności.</li>
  <li>Konstrukcje bezosobowe skupiają uwagę na zjawisku, zasadzie lub wyniku.</li>
  <li>Przeczenie odrzuca czynność, stan albo modalność.</li>
  <li>W zdaniach złożonych każde centrum pomaga wyznaczyć osobną warstwę informacji.</li>
  <li>Wybór typu orzeczenia zmienia punkt widzenia, tempo i sposób przedstawiania odpowiedzialności.</li>
</ul>
</div>

[Wróć do przeglądu orzeczenia](../){ .md-button }

</div>